...
class MultiscaleGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if image is None or label is None or non_expert is None:
            print(f"Error: Could not read image, label or non_expert")
            return
...
class MicroUS_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            if label is None:
                print(f"Error: Could not read label at {label_path}")
            else:
                label = label[:,:,0]/255.0
            non_expert = cv2.imread(non_expert_path)
            if non_expert is None:
                print(f"Error: Could not read non_expert at {non_expert_path}")
            else:
                non_expert = non_expert[:,:,0]/255.0
